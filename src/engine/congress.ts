...
export function createSampleBill(turn: number): Bill {
  return {
...
export function voteBill(bill: Bill, playerVote: 'for' | 'against' | 'abstain'): Bill {
  let forCount = bill.votesFor;
  let againstCount = bill.votesAgainst;
  let abstainCount = bill.votesAbstain;

  if (playerVote === 'for') forCount += 1;
  else if (playerVote === 'against') againstCount += 1;
  else abstainCount += 1;

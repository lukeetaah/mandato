...
export const OnboardingModal = ({ isOpen, onClose }) => {
    ...
    return (_jsx(Modal, { isOpen: isOpen, onClose: onClose, title: currentPage.title, children: _jsxs("div", { className: "space-y-4 text-sm text-slate-300 leading-relaxed min-h-[280px]", children: [_jsx("div", { className: "flex justify-center gap-2 mb-2", children: LORE_PAGES.map((_, idx) => (_jsx("div", { className: `w-2 h-2 rounded-full transition-all ${idx === page ? 'bg-amber-400 w-6' : idx < page ? 'bg-sky-400' : 'bg-slate-700'}` }, idx))) }), _jsx("div", { className: "whitespace-pre-line text-[13px] leading-relaxed", children: currentPage.content.split('**').map((part, idx) => (idx % 2 === 1
                        ? _jsx("strong", { className: "text-sky-300 font-semibold", children: part }, idx)
                        : _jsx("span", { children: part }, idx))) }), _jsxs("div", { className: "pt-4 border-t border-slate-800 flex justify-between items-center", children: [page > 0 ? (_jsx(Button, { variant: "ghost", size: "sm", onClick: () => setPage(page - 1), children: "\u2190 Anterior" })) : (_jsx("div", {})), isLastPage ? (_jsx(Button, { variant: "gold", size: "md", onClick: onClose, children: "Iniciar Mandato \u2794" })) : (_jsx(Button, { variant: "primary", size: "md", onClick: () => setPage(page + 1), children: "Siguiente \u2192" }))] })] }) }));
};
